...
async function loadPreview() {
  try {
    const res = await axios.get(`/api/clan-event-videos/${clanSlug.value}/preview`, {
      params: {
...
async function loadList() {
  loading.value = true
  try {
    const res = await axios.get(`/api/clan-event-videos/${clanSlug.value}`)
...
async function handleGenerate() {
  ...
  try {
    const res = await axios.post(`/api/clan-event-videos/${clanSlug.value}`, {
      ...form,
      start_year: form.start_year || undefined,
      end_year: form.end_year || undefined,
...
function startPolling(projectId: string) {
  ...
  pollingTimer.value = window.setInterval(async () => {
    try {
      const res = await axios.get(`/api/clan-event-videos/${clanSlug.value}/${projectId}`)
